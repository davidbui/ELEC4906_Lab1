...
float scale_scale[] = { 0.25, 0.5, 1, 2, 4};
int current_ss = 2;
...
void play_twinkle_twinkle_little_star()
{
	...
	for (i=0; i<42; i++)
	{
			WTIMER0->TAILR = (uint32_t)twinkle_twinkle_little_star_notes[i]*scale_scale[current_ss];
...
void ADC0SS3_Handler(void)
{
	...
	if (adc0Result > 3279)
		current_ss = 4;
	else if(adc0Result > 2461)
		current_ss = 3;
	else if(adc0Result > 1643)
		current_ss = 2;
	else if(adc0Result > 825)
		current_ss = 1;
	else {
		current_ss = 0;
	}
